...
function OnTriggerStay (other : Collider){
	...
	if (canLeave)
	{
		...
		if(Input.GetKeyDown(KeyCode.LeftArrow) || Input.GetKeyDown(KeyCode.RightArrow) || Input.GetKeyDown(KeyCode.RightArrow)|| Input.GetKeyDown(KeyCode.RightArrow)){
		
			ToggleConversation(other);
		}
	}
	else if (Input.GetKeyDown("1")) //TODO: change to do quiz here
	{
		ToggleConversation(other);
		if (canReward && (other.name == "Player")) {
			other.BroadcastMessage("addReward", 50, SendMessageOptions.DontRequireReceiver);
		}
	}
	
}

function ToggleConversation(other : Collider) {
